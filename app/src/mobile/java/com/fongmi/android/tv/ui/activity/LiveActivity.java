...
public class LiveActivity extends PlaybackActivity implements CustomKeyDown.Listener, TrackDialog.Listener, Biometric.Callback, PassCallback, ConfigCallback, LiveCallback, GroupAdapter.OnClickListener, ChannelAdapter.OnClickListener, EpgDataAdapter.OnClickListener, CastDialog.Listener, InfoDialog.Listener {
    ...
    private void setVideoView() {
        ...
        mBinding.video.addOnLayoutChangeListener((view, left, top, right, bottom, oldLeft, oldTop, oldRight, oldBottom) -> {
            if ((left != oldLeft || top != oldTop || right != oldRight || bottom != oldBottom) && !mEnteringPiP && !isInPictureInPictureMode()) {
    ...
    private void setScale(int scale) {
        ...
        if (service() != null) player().setScale(scale);
        ...
        try {
            ...
            mNetworkCallback = new ConnectivityManager.NetworkCallback() {
                ...
                public void onAvailable(@NonNull Network network) {
                    ...
                    if (mCurrentNetwork.equals(network)) return;
                    mCurrentNetwork = network;
                    App.post(() -> {
    ...
    private void unregisterNetworkCallback() {
        try {
            ConnectivityManager cm = (ConnectivityManager) getSystemService(Context.CONNECTIVITY_SERVICE);
            if (cm != null && mNetworkCallback != null) cm.unregisterNetworkCallback(mNetworkCallback);
        } catch (Exception ignored) {}
    }
    ...
    protected void onUserLeaveHint() {
        ...
        if (player().haveTrack(C.TRACK_TYPE_VIDEO)) {
            mEnteringPiP = true;
