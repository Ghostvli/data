...
public class PlayerManager implements ParseCallback {
    ...
    private void switchEngine(int engineType) {
        if (engine != null) {
        ...
        evictConnections();
        if (engineType == 0) {
            engine = new ExoPlayerEngine(PlayerEngine.HARD, listener);
            player = engine.getPlayer();
        } else {
            engine = new MpvPlayerEngine(Setting.getMpvDecode(), listener);
            player = engine.getPlayer();
        }
    ...
    private int getCurrentEngineType() {
        return (engine instanceof MpvPlayerEngine) ? 1 : 0;
    }

    public void ensureMpv() {
        if (isMpvSupported() && getCurrentEngineType() != 1) {
            switchEngine(1);
        }
    }

    public void setForceMpv(boolean force) {
        this.forceMpv = force;
    }

    private void checkEngine(String url) {
        int targetEngine = Setting.getPlayerEngine();
        if (forceMpv) {
            targetEngine = 1;
        } else if (url != null && (url.toLowerCase().startsWith("rtsp://") || url.toLowerCase().contains("rtsp") || url.contains(":554") || url.toLowerCase().startsWith("udp://") || url.toLowerCase().startsWith("rtp://") || url.toLowerCase().contains("zte"))) {
            targetEngine = 1;
        }
        if (targetEngine == 1 && !isMpvSupported()) {
            targetEngine = 0;
        }
        if (getCurrentEngineType() != targetEngine) {
    ...
    public boolean isExo() {
        return engine instanceof ExoPlayerEngine;
    }
    ...
    public void play() {
        engine.play();
        if (!isMpv()) player.play();
    }

    public void pause() {
        engine.pause();
        if (!isMpv()) player.pause();
    }

    public void stop() {
        if (danPlayer != null) danPlayer.stop();
        engine.stop();
        if (!isMpv()) player.stop();
        stopParse();
        evictConnections();
    }
    ...
    public void toggleEngine() {
        int target = getCurrentEngineType() == 0 ? 1 : 0;
        if (target == 1 && !isMpvSupported()) {
            com.fongmi.android.tv.utils.Notify.show("当前设备不支持 MPV 解码器");
            return;
        }
        if (danPlayer != null) danPlayer.stop();
        engine.stop();
        ...
        if (spec != null && spec.getUrl() != null) {
            ...
            if (!isMpv()) App.post(runnable, Constant.TIMEOUT_PLAY);
            callback.onPrepare();
            initTrack = false;
        }
    }
    ...
    private void setMediaItem(long timeout) {
        ...
        checkEngine(spec.getUrl());
        setDanmakus(spec.getDanmakus());
        engine.start(spec.checkUa());
        if (!isMpv()) App.post(runnable, timeout);
        callback.onPrepare();
        initTrack = false;
    }
    ...
    private void evictConnections() {
        try {
            com.github.catvod.net.OkHttp.cancelAll(com.github.catvod.net.OkHttp.player());
        } catch (Throwable ignored) {
        }
        try {
            com.github.catvod.net.OkHttp.player().connectionPool().evictAll();
        } catch (Throwable ignored) {
        }
    }

    public static boolean isMpvSupported() {
        try {
