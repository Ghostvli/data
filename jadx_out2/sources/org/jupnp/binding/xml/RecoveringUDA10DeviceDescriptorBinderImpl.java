    ...
    public String fixMissingNamespaces(String str, DescriptorBindingException descriptorBindingException) {
        ...
        if (!matcher.find() || matcher.groupCount() != 1) {
            ...
            if (matcher.find() && matcher.groupCount() == 1) {
                ...
                SpecificationViolationReporter.report("Fixing missing namespace declaration for: {}", strGroup);
                Matcher matcher2 = Pattern.compile("<root([^>]*)").matcher(str);
                if (matcher2.find() && matcher2.groupCount() == 1) {
                    String strGroup2 = matcher2.group(1);
                    this.logger.h("Preserving existing <root> element attributes/namespace declarations: {}", matcher2.group(0));
                    Matcher matcher3 = Pattern.compile("<root[^>]*>(.*)</root>", 32).matcher(str);
                    if (!matcher3.find() || matcher3.groupCount() != 1) {
                        this.logger.y("Could not extract body of <root> element");
                        return null;
                    }
                    String strGroup3 = matcher3.group(1);
                    return "<?xml version=\"1.0\" encoding=\"UTF-8\" ?><root " + String.format("xmlns:%s=\"urn:schemas-dlna-org:device-1-0\"", strGroup) + strGroup2 + ">" + strGroup3 + "</root>";
                }
                this.logger.y("Could not find <root> element attributes");
            }
        }
        return null;
    }
