...
public class Parser {
    ...
    /* JADX DEBUG: Don't trust debug lines info. Lines numbers was adjusted: min line is 1 */
    /* JADX DEBUG: Incorrect finally slice size: {[GOTO, IPUT, IPUT, CONST_STR, INVOKE] complete}, expected: {[GOTO, IPUT, IPUT] complete} */
    ...
    */
        ...
        try {
            ...
            if (iNextToken != 44) {
                if (iNextToken == 57) {
                    ...
                    int i5 = mustMatchToken(97, "msg.no.paren.for.ctrl", true) ? this.ts.tokenBeg - i3 : -1;
                    arrayComprehensionLoop.setLength(this.ts.tokenEnd - i3);
            ...
            } else if ("of".equals(this.ts.getString())) {
                if (i != -1) {
            ...
            i2 = -1;
            z = false;
            AstNode astNodeExpr22 = expr(false);
            if (mustMatchToken(97, "msg.no.paren.for.ctrl", true)) {
            }
            arrayComprehensionLoop.setLength(this.ts.tokenEnd - i3);
            arrayComprehensionLoop.setIterator(astNodeCreateNameNode);
            arrayComprehensionLoop.setIteratedObject(astNodeExpr22);
            arrayComprehensionLoop.setInPosition(i2);
            arrayComprehensionLoop.setEachPosition(i);
            if (i == -1) {
            }
    ...
    private RuntimeException codeBug() {
        TokenStream tokenStream = this.ts;
        throw Kit.codeBug("ts.cursor=" + tokenStream.cursor + ", ts.tokenBeg=" + tokenStream.tokenBeg + ", currentToken=" + this.currentToken);
    }
    ...
    private AstNode createNumericLiteral(int i, boolean z) {
        ...
        if (i == 89) {
            numberLiteral = new BigIntLiteral(this.ts.tokenBeg, string + "n", this.ts.getBigInt());
        } else {
            TokenStream tokenStream = this.ts;
    ...
    private AstNode defaultXmlNamespace() {
        ...
        int iColumnNumber = columnNumber();
        int i = this.ts.tokenBeg;
        if (!matchToken(44, true) || !XMLConstants.XML_NS_PREFIX.equals(this.ts.getString())) {
    ...
    /* JADX DEBUG: Don't trust debug lines info. Lines numbers was adjusted: min line is 1 */
    /* JADX DEBUG: Failed to insert an additional move for type inference into block B:53:0x000b */
    /* JADX DEBUG: Failed to insert an additional move for type inference into block B:58:0x000b */
    ...
    /* JADX DEBUG: Don't trust debug lines info. Lines numbers was adjusted: min line is 1 */
    /* JADX DEBUG: Failed to insert an additional move for type inference into block B:112:0x01b2 */
    /* JADX DEBUG: Multi-variable search result rejected for r19v0, resolved type: org.mozilla.javascript.Parser */
    /* JADX DEBUG: Multi-variable search result rejected for r2v3, resolved type: org.mozilla.javascript.ast.ObjectLiteral */
    ...
    /* JADX DEBUG: Don't trust debug lines info. Lines numbers was adjusted: min line is 1 */
    /* JADX DEBUG: Failed to insert an additional move for type inference into block B:58:0x007a */
    ...
    */
    private TryStatement tryStatement() {
        ...
        int iLineNumber = lineNumber();
        int iColumnNumber = columnNumber();
        TryStatement tryStatement = new TryStatement(i9);
        int iPeekToken = peekToken();
        while (true) {
            z = true;
            if (iPeekToken != 175) {
            ...
            while (matchToken(i11, z)) {
                ...
                try {
                    if (iPeekToken3 == i10) {
                        ...
                        statements(scope);
                        popScope();
                        int nodeEnd2 = getNodeEnd(scope);
                        catchClause.setVarName(nameCreateNameNode);
                        catchClause.setCatchCondition(astNode);
                        catchClause.setBody(scope);
                        if (i7 != -1) {
                            catchClause.setIfPosition(i7 - i12);
                        }
                        catchClause.setParens(i6, i5);
                        int i14 = !mustMatchToken(95, "msg.no.brace.after.body", true) ? this.ts.tokenEnd : nodeEnd2;
                        catchClause.setLength(i14 - i12);
                        if (arrayList2 != null) {
                    ...
                    statements(scope);
                    popScope();
                    int nodeEnd22 = getNodeEnd(scope);
                    catchClause.setVarName(nameCreateNameNode);
                    catchClause.setCatchCondition(astNode);
                    catchClause.setBody(scope);
                    if (i7 != -1) {
                ...
                nameCreateNameNode = null;
                astNode = null;
                Scope scope2 = new Scope(i12);
                boolean z52 = z3;
                CatchClause catchClause2 = new CatchClause(i12);
                catchClause2.setLineColumnNumber(iLineNumber2, iColumnNumber2);
                pushScope(scope2);
            }
            i = i9;
            z2 = z;
    ...
    /* JADX DEBUG: Don't trust debug lines info. Lines numbers was adjusted: min line is 1 */
    /* JADX DEBUG: Multi-variable search result rejected for r2v0, resolved type: org.mozilla.javascript.ast.AstNode */
    /* JADX WARN: Multi-variable type inference failed */
